...
library LibDiamond {
    ...
    event OwnershipTransferStarted(
        address indexed owner, address indexed pendingOwner
    );
    ...
    function enforceIsPendingOwner() internal view {
        if (msg.sender != diamondStorage().pendingOwner) {
            require(
                msg.sender == diamondStorage().pendingOwner,
                "LibDiamond: Must be pending owner"
            );
        }
    }
    ...
    function startTransferOwnership(address newOwner) internal {
        ...
        emit OwnershipTransferStarted(ds.contractOwner, ds.pendingOwner);
    }

    event DiamondCut(
        IDiamondCut.FacetCut[] _diamondCut, address _init, bytes _calldata
    );
        ...
        internal
    {
        enforceHasContractCode(
            _facetAddress, "LibDiamondCut: New facet has no code"
        );
        ...
        internal
    {
        if (_init == address(0)) {
            ...
            if (_init != address(this)) {
                enforceHasContractCode(
                    _init, "LibDiamondCut: _init address has no code"
                );
            }
